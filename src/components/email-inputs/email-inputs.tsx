...
const EmailInputs  =({emails,onChange}: props) => {
  ...
  const onKeyDown = (e:KeyboardEvent<HTMLInputElement>) => {
    //@ts-ignore
    if (e.code==="Enter"&&!emails.includes(input)&&!e.nativeEvent.isComposing&&e.currentTarget.validity.valid){
      e.preventDefault();
  ...
  return <label className={Styles.wrapper} htmlFor={Styles.input}>
    {emails.map((email) => <Email key={email} email={email} onClick={()=>onChange(removeItemFromArray(emails,email))}/>)}
    <input id={Styles.input} placeholder={"example@example.com"} className={Styles.input} type="email" value={input} onChange={(e)=>setInput(e.target.value)} onKeyDown={onKeyDown}/>
  </label>
}
